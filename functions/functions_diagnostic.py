...
def CrossVal2(X, Y, patient_name, BATCH_SIZE=32, nfold=0, seed=1):
...
def RunAutoEncoder_old(
    ...
    for e in range(N_EPOCHS):
        ...
        epoch_classification.append(running_classification / train_len)
        epoch_acc.append(running_accuracy / train_len)
        print(running_accuracy / train_len)
        if running_accuracy > best_test:
            best_net_it = e
            best_test = running_accuracy
            torch.save(net.state_dict(), str(outputPath) + "/best_net")
...
def ShowPcaTsne(
    X, Y, data_encoder, center_distance, class_len, tit, pcafit=None, test_legends=None
):
...
def ReadData(
    file_name, file_name2=None, TIRO_FORMAT=True, model="", doScale=False, doLog=False,
):
    """Read different data(csv, npy, mat) files  
    * csv has two format, one is data of facebook, another is TIRO format.
    
    Args:
        file_name: string - file name, default directory is "data/"
    ...
    if file_name2 is None:
        return (X, Y, feature_name, label_name, patient_name, None)
    return (
        X,
        Y,
        feature_name,
