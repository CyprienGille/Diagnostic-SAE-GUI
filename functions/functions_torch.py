...
def proj_l21ball(w2, eta, axis=None, device="cpu"):
    ...
    if w.dim() == 1:
        ...
        # W = torch.tensor(
        #     [torch.linalg.norm(w[:, i], ord=2).data.item() for i in range(ncol)]
        # )

        PW = proj_l1ball(W, eta, device=device)
        # PW = proj_l2ball(W, eta, device=device)
...
def proj_l1infball(w0, eta, device="cpu"):
    w = torch.as_tensor(w0, dtype=torch.get_default_dtype(), device="cpu")
    init_size = w.shape
    ...
    X = torch.sort(torch.abs(w), dim=1, descending=True)[0]
    v = torch.sum(X[:, 0])
    if v <= eta:
        print(f"Layer of size {init_size} was inside ball of radius {eta}")
        return w0.clone().detach().requires_grad_(True).to(device)
    ...
    theta = (v - eta) / N
    mu = torch.zeros((N, 1))
    active = torch.ones((N, 1))
    theta_old = torch.tensor(0)
    while abs(theta_old - theta) > 1e-7:
        for n in range(N):
            if active[n]:
                j = idx[n].clone()
                while (j < M - 1) and (
                    ((S[n, j.long()] - theta) / j) < X[n, j.long() + 1]
                ):
                    j += 1
                idx[n] = j.clone()
                mu[n] = S[n, j.long()] / (j + 1)
                if (j + 1) == M and ((mu[n] - (theta / (j + 1))) <= 0):
                    active[n] = 0
                    mu[n] = 0
        theta_old = theta
        theta = (torch.sum(mu) - eta) / torch.sum(torch.divide(active, idx))

    Xp = torch.divide(mu - theta, idx)
    Xp = torch.multiply(Xp, active)
    X = torch.minimum(torch.abs(w), Xp)
    X = torch.multiply(torch.sign(w), X)
    X = torch.squeeze(X)
